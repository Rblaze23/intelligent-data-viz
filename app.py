...
def convert_llm_to_viz_specs(
    llm_result: Dict[str, Any], data: pd.DataFrame
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for viz in llm_visualizations:
            ...
            spec = {
                ...
                "description": viz.get(
                    "justification", "LLM-recommended visualization"
                ),
            ...
            if y_axis in valid_cols:
                ...
                spec["y_col"] = (
                    numeric_cols[0]
                    if numeric_cols
                    else (data.columns[1] if len(data.columns) > 1 else None)
                )
...
def recommend_kpis(data: pd.DataFrame) -> Dict[str, List[str]]:
    ...
    if numeric_cols:
        for col in numeric_cols[:3]:
            kpis["Performance Metrics"].extend(
                [f"Average {col}", f"Median {col}", f"Max {col}", f"Min {col}"]
            )
        ...
        kpis["Distribution Metrics"].extend(
            [
                f"Standard deviation of {numeric_cols[0]}"
    ...
    if categorical_cols and numeric_cols:
        kpis["Category Performance"].extend(
            [
                f"Performance by {categorical_cols[0]}"
                if len(categorical_cols) > 0
                else None,
                f"Top performing {categorical_cols[0]}"
                if len(categorical_cols) > 0
                else None,
...
def generate_visualizations_from_llm(
    ...
    try:
        ...
        logger.info(
            "Step 1/3: LLM Analyzer generating visualization recommendations..."
        )
        llm_result = analyzer.analyze_and_recommend(problem, data)

        # Step 2: Convert LLM output to Plotly specs (harmonization)
        logger.info(
            "Step 2/3: Converting LLM output to visualization specifications..."
        )
        ...
        for spec in viz_specs:
            try:
                ...
                logger.error(
                    f"Failed to generate {spec.get('type', 'unknown')} visualization: {str(e)}"
                )
                ...
                try:
                    ...
                    valid_specs.append(spec)
                    logger.info(
                        f"Added fallback figure for {spec.get('type', 'unknown')}"
                    )
                except Exception as fallback_err:
                    logger.error(
                        f"Failed to create fallback figure: {str(fallback_err)}"
                    )

        logger.info(
            f"Successfully generated {len(figures)} visualizations (including {len(figures) - len(valid_specs)} fallbacks)"
        )
...
def generate_dashboard_visuals(
    dashboard_spec: Dict[str, Any], data: pd.DataFrame
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if kpi_cards:
            ...
            fig_kpi = go.Figure(
                data=[
                    go.Bar(
                        ...
                        textfont=dict(
                            size=11, family="Arial, sans-serif", color="#333"
                        ),
            ...
            fig_kpi.update_layout(
                title=dict(
                    text="<b>Key Performance Indicators (KPI Summary)</b>",
                ...
                yaxis=dict(
                    gridcolor="rgba(0,0,0,0.05)", showgrid=False, zeroline=False
                ),
        ...
        if business_metrics:
            ...
            fig_metrics = go.Figure(
                data=[
                    go.Bar(
                        ...
                        textposition="outside",
                        textfont=dict(
                            size=10, family="Arial, sans-serif", color="#333"
                        ),
            ...
            fig_metrics.update_layout(
                title=dict(
                    text="<b>Business Metrics Overview</b>",
                    font=dict(size=14, color="#1a1a1a"),
                ),
                xaxis_title="Metrics",
        ...
        if insights:
            ...
            fig_insights = go.Figure(
                layout=go.Layout(
                    ...
                    title=dict(
                        text="<b>Key Insights & Findings</b>",
                        font=dict(size=14, color="#1a1a1a"),
                    ),
            ...
            if filter_names:
                fig_filters = go.Figure(
                    go.Sunburst(
                        ...
                        textfont=dict(
                            size=11, family="Arial, sans-serif", color="white"
                        ),
        ...
        fig_specs.update_layout(
            polar=dict(
                ...
                angularaxis=dict(
                    tickfont=dict(size=10, color="#333"), gridcolor="rgba(0,0,0,0.1)"
                ),
                bgcolor="rgba(250,250,250,0.5)",
            ),
            title=dict(
                text="<b>Dashboard Quality Score</b>",
                font=dict(size=14, color="#1a1a1a"),
            ),
            height=450,
            template="plotly_white",
        ...
        logger.info(
            f"Generated {len(visuals)} professional dashboard visual components"
        )
        return visuals
...
def main():
    ...
    # Note: set_page_config is already called at top of file (line 49)
    ...
    with st.container():
        ...
        uploaded_file = st.file_uploader(
            "Choose a CSV file", type=["csv"], help="Upload a CSV file with your data"
        )
        ...
        if uploaded_file is not None:
            try:
                ...
                st.write(
                    "**Custom KPIs:** Add your own by typing in the problem statement below"
                )
        ...
        if st.session_state.data is not None:
            ...
            if st.session_state.problem_statement:
                ...
                if st.button("🚀 Generate Visualizations", key="generate_btn"):
                    with st.spinner(
                        "Analyzing data with AI and generating visualizations..."
                    ):
                        try:
                            ...
                            if figures:
                                st.success(
                                    f"✅ Generated {len(figures)} visualization proposals!"
                                )
                            else:
                                UIComponents.error_message(
                                    "Could not generate any visualizations"
                                )
                                return
                ...
                if st.session_state.visualizations:
                    ...
                    titles = [
                        spec.get("title", f"Option {i+1}")
                        for i, spec in enumerate(st.session_state.get("viz_specs", []))
                    ]
                    descriptions = [
                        spec.get("description", "LLM-recommended visualization")
                        for spec in st.session_state.get("viz_specs", [])
                    ]
                    justifications = [
                        spec.get("justification", "Based on LLM analysis")
                        for spec in st.session_state.get("viz_specs", [])
                    ]
                    ...
                    if selected_idx >= 0:
                        ...
                        if st.button(
                            ...
                            with st.spinner(
                                "Creating dashboard specification with VLM..."
                            ):
                                try:
                                    components = get_components()
                        ...
                        if st.session_state.get("dashboard_spec"):
                            ...
                            if isinstance(dashboard, list):
                                UIComponents.error_message(
                                    "Dashboard specification format error"
                                )
                            else:
                                # Dashboard Overview
                                st.subheader(
                                    dashboard.get("dashboard_title", "Dashboard")
                                )
                                ...
                                try:
                                    ...
                                    logger.error(
                                        f"Error generating visual dashboard: {str(e)}"
                                    )
                                    UIComponents.error_message(
                        ...
                        with col1:
                            if st.button(
                                "📥 Export PNG (High Quality)", key="export_png"
                            ):
                                try:
                        ...
                        with col2:
                            if st.button(
                                "📥 Export HTML (Interactive)", key="export_html"
                            ):
                                try:
