...
class DataValidator:
    ...
    def check_data_quality(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if missing_percent > 50:
            quality_report["warnings"].append(f"High missing values: {missing_percent:.1f}%")
        
        # Check for constant columns (all same value)
        for col in df.columns:
            if df[col].nunique() == 1:
                quality_report["warnings"].append(f"Column '{col}' has only one unique value")
    ...
    def validate_visualization_columns(
        ...
        if viz_type == "scatter_plot":
            ...
            if not pd.api.types.is_numeric_dtype(df[x_axis]):
                raise ValueError(f"X-axis column '{x_axis}' must be numeric for scatter plot")
            if not pd.api.types.is_numeric_dtype(df[y_axis]):
                raise ValueError(f"Y-axis column '{y_axis}' must be numeric for scatter plot")
            ...
            if not pd.api.types.is_numeric_dtype(df[x_axis]):
                raise ValueError(f"X-axis column '{x_axis}' must be numeric for histogram")
