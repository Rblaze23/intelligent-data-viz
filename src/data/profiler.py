...
class DataProfiler:
                ...
                profile.update(
                    {
                        ...
                        "true_percent": float(
                            (value_counts.get(True, 0) / len(col_data)) * 100
                        ),
                    }
                )
    ...
    def _get_data_quality(self) -> Dict[str, Any]:
        ...
        quality = {"overall_quality": "good", "issues": [], "warnings": []}
        ...
        if missing_percent > 50:
            ...
            quality["warnings"].append(
                f"Moderate missing values: {missing_percent:.1f}%"
            )
    ...
    def _get_recommendations(self) -> List[str]:
        ...
        if len(datetime_cols) >= 1 and len(numeric_cols) >= 1:
            recommendations.append("Line charts are ideal for showing trends over time")
...
if __name__ == "__main__":
    ...
    for col, col_prof in profile["column_profiles"].items():
        ...
        if col_prof["type"] == "numeric":
            ...
            print(
                f"      Most common: {col_prof['most_common']} ({col_prof['most_common_freq']})"
            )
